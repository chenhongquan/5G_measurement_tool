...
if(len(sys.argv)<4):
	print sys.argv[0],"filename :interval write_file"
	exit(0)
...
#for line in sys.stdin: 
	...
	if("local" in line and "connected to" in line):
		...
		serverIp=param[3]
		clientIp=param[8]
		serverPort=int(param[5])
		clientPort=int(param[10])		
	...
	if("Starting Test" in line and start_file==0):
		...
		start_file=1
		if(dip_count.has_key(clientIp+serverIp+str(serverPort))==0):
			dip_count[clientIp+serverIp+str(serverPort)]=1
		else:
	...
	if("mss" in line and "cwnd" in line and "totalretx" in line and processCount<=streamsCount*(duration/interval) and cwnd_write==0):
		...
		if(processCount==streamsCount*((duration-1)/interval)):
			#print "closing file"
			#fdWrite.close()
			continue
	...
	if("sec" in line and "Bytes" in line and "bits/sec" in line and "SUM" not in line and "sender" not in line and "receiver" not in line and thrProcessCount<=streamsCount*(duration/interval) and thr_write==0):
		...
		if(thrProcessCount==streamsCount*((duration-1)/interval)):
			continue

		param=line.split()
		thr=float(param[6])
		if(param[7]=="Mbits/sec"): thr=thr
		if(param[7]=="Kbits/sec"): thr=thr/1000
		if(param[7]=="bits/sec"): thr=thr/1000000
		#print thr
