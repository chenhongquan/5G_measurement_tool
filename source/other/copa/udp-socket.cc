...
int UDPSocket::bindsocket(string s_ipaddr, int s_port, int sourceport){
  ...
  struct sockaddr_in srcaddr;
	...
	srcaddr.sin_addr.s_addr = inet_addr("000.000.000.000"); // change server IP
	...
	if (::bind(udp_socket, (struct sockaddr *)&srcaddr, sizeof(srcaddr)) < 0) {
		...
		bound = true;
  		cout<<"syslab: I am creating socket and binding it to a address\t"<<s_ipaddr<<"port \t"<<srcport<<"\n";
		return 0;
  }
}
...
ssize_t UDPSocket::senddata(const char* data, ssize_t size){
	...
	int res = sendto(udp_socket, data, size, 0, (struct sockaddr *) &sandy_dest_addr, sizeof(sandy_dest_addr));
	...
	if ( res == -1 ){
		...
		//std::cout<<"syslab: sent the data to receiver back\n";
	}
...
int UDPSocket::receivedata(char* buffer, int bufsize, int timeout, sockaddr_in &other_addr){
	//assert(bound); // Socket not bound to an address. Please either use 'bind' or 'sendto' sandy commented it out
	...
	if( poll_val == 1){
		...
		std::cerr<<"\nThere was an error while polling. Code: "<<errno<<endl;
		return -1;
	}
	else{
